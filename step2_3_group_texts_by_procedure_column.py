...
# -*- coding: utf-8 -*-
"""
step2.3 - 基于坐标按列组织手术及操作表格内容

该脚本处理OCR识别结果，提取"手术及操作名称"到"离院方式"之间的文本内容，
基于X坐标范围按列进行组织，适用于手术及操作表格的文本数据。
"""
...
# ================== 配置参数 ==================
# 关键字段标识
START_MARKER = "手术及操作名称"
END_MARKER = "高院方式"  # OCR识别可能有误，应该是"离院方式"
ALTERNATIVE_END_MARKER = "1.医瞩离院"  # 备选结束标记

# ================== 辅助函数 ==================

def extract_text_range_excluding_end(ocr_results, start_marker, end_marker, alternative_end_marker):
    """
    从OCR结果中提取指定范围内的文本内容，基于表格结构
    只提取"手术及操作名称"表格内的内容，不包括后续的独立行
    
    参数:
        ocr_results: OCR识别结果列表
        start_marker: 开始标记文本
        end_marker: 结束标记文本
        alternative_end_marker: 备选结束标记文本
    ...
    for item in ocr_results:
        ...
        # 检查是否找到开始标记
        if not start_found and start_marker in text:
            start_found = True
        ...
        if start_found:
            # 检查是否找到结束标记
            if end_marker in text or alternative_end_marker in text:
                end_found = True
                break  # 找到结束标记，停止提取
            
            # 检查Y坐标是否超出表格范围（与开始标记Y坐标差异过大）
            # 如果Y坐标差异超过200像素，认为已经超出表格范围
            if start_y is not None and abs(y_center - start_y) > 200:
                # 检查当前文本是否可能是新的表格标题
                if any(keyword in text for keyword in ['高院方式', '离院方式', '出院诊断', '病理诊断', '药物过敏']):
                    end_found = True
                    break  # 超出表格范围，停止提取
...
def group_texts_by_column(text_blocks):
    ...
    for block in processed_blocks:
        if current_y is None:
            ...
            # 如果Y坐标差异小于阈值，认为是同一行
            if abs(block['y_center'] - current_y) < 25:  # 行阈值（手术表格行间距可能较大）
...
def save_column_results(columns, column_average_x_coordinates, output_path, metadata):
    ...
    for i, column in enumerate(columns):
        avg_x = column_average_x_coordinates[i] if i < len(column_average_x_coordinates) else 0.0
...
class GroupTextsByProcColumn:
    """主程序入口"""
    def __init__(self, input_path=None):
        self.input_path = input_path or os.path.join(os.path.dirname(os.path.abspath(__file__)), 'output', 'step1', '5_eye_Page2_2.0.json')
        self.output_path = "DRG_model\output\step2.3\step2_3_proc_column.json"
    
    # 获取当前脚本所在目录
    # script_dir = os.path.dirname(os.path.abspath(__file__))
    
    # # 输入输出路径配置（相对于脚本目录）
    # input_path = os.path.join(script_dir, 'output', 'step1', '5_eye_Page2_2.0.json')
    # output_path = os.path.join(script_dir, 'output', 'step2.3', '5_eye_Page2_2.0_procedure_column_grouped.json')
    ...
    def run(self):
        ...
        extracted_texts, start_found, end_found = extract_text_range_excluding_end(
            all_text_blocks, START_MARKER, END_MARKER, ALTERNATIVE_END_MARKER
        )
        ...
        if not end_found:
            print(f"警告: 未找到结束标记 '{END_MARKER}' 或 '{ALTERNATIVE_END_MARKER}'")
        ...
        metadata = {
            ...
            'alternative_end_marker': ALTERNATIVE_END_MARKER,
        ...
        print(f"  - 备选结束标记: {ALTERNATIVE_END_MARKER}")
...
#     group_tex = GroupTextsByProcColumn()
#     group_tex.run()
